...
cpDat%>%
  ...
  add_header_above(c(" " = 2, "# Reassignments" = 5))
sink()
...
pack_rows("Pretest",1,2)%>%pack_rows("Race/Ethnicity",3,5)%>%pack_rows("Sex",6,7)%>%pack_rows("Grade",8,9)%>%
  add_header_above(c(" " = 1, "Before Match" = 2, "After Match"=2))
sink()
...
cpDat2 <- full_join(cpDat2,tibble(classid2=cpDatClass$classid2,match=paste0('c',as.character(cpDatClass$m2))))

balClass <- balanceTest(everCP~mpretest+race+sex+grade+spec_speced+spec_gifted+spec_esl+frl+frlMIS+year+state+cluster(classid2)+strata(match),data=cpDat2,report=c("std.diffs","z.scores","chisquare.test"))

mod1c <- lm_robust(y_yirt~everCP,fixed_effects=match,clusters=cpDat2$classid2,data=cpDat2)
mod2c <- update(mod1c,.~.+year+state)
...
save(dataInfo,eff,effSE,mod0,mod1,mod2,mod3.1,mod3.2,mod4,sens,mod1c,mod2c,mod1t,mod2t,studOverall,psmodSumm, match4,studOverall,studCovs,balClass,sdEff,sdEffp,sdSlope,sdSlopep,psmod2coef,file='artifacts/dataAnalysis.RData')
